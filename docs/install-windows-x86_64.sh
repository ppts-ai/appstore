...
echo "Creating systemd service file at $SERVICE_FILE3"
cat <<EOL > "$SERVICE_FILE3"
# container-ssh-proxy.service
# autogenerated by Podman 5.3.0-dev-e8410b839
# Tue Dec 10 12:56:11 CET 2024

[Unit]
Description=Podman container-ssh-proxy.service
...
ExecStart=/usr/bin/podman run \
	...
	-p 2222:2222 \
	-p 7376:7376 \
	-v /etc/podman/nginx.conf:/etc/nginx/nginx.conf nginx
...
[Service]
ExecStart=/bin/sh -c 'exec $EXECUTABLE_PATH --mapping tcp:0.0.0.0:2022-127.0.0.1:$3 -n "$1" -k "$2" ${4:+-w "$4"}'
Restart=always
User=root
